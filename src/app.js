...
const { day, month, year, dateStr, hours, mins, secs, a } = getTime();
const timeStr = `${dateStr} ${hours}:${mins} ${a}`;
  ...
  try {
    ...
    if (!passwordAuth) {
      ...
      if (user.status === "Approved") {
        ...
        fs.appendFileSync('./uploads/Admin/Users.csv', `${roll} , Logged in , ${timeStr} \n`)
      ...
      if (pass != cpass) {
        ...
        if (checkRollCount > 0) {
          ...
          if (!nametoken && !batchtoken && !rollnotoken && !verifytoken && !branchtoken) {
            ...
            fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Requested for Registration , ${timeStr} \n`)
            res.render("registeralumni", {
              success:
            ...
            transporter.sendMail(mailOptions, (error, info) => {
              if (error) {
                ...
                console.log(`Email Sent to ${roll}`);
              }
            });
          }
          else {
            if (nametoken === name && batchtoken === batch && rollnotoken === rollno && branchtoken === branch) {
              ...
              fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Registration Successfull , ${timeStr} \n`)
              const userget = await Register.findOne({ rollno: roll }, { mailid: 1, password: 1, name: 1 })
              const mailid = userget.mailid
              const password = userget.password
              const name = userget.name
...
app.post('/forgotpassword', async (req, res) => {
  ...
  if (countroll > 0) {
    ...
    try {
      ...
      fs.appendFileSync('./uploads/Admin/Users.csv', `${rollnumber} , Requested for Password , ${timeStr} \n`)
      res.render('loginalumni', {
  ...
  if (tokenStore.has(token)) {
    ...
    try {
      ...
      if (complaintscount < 2) {
        ...
        fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Registred a new Complaint (${complaintId}) , ${timeStr} \n`)
...
app.post("/post/:id/addcomment", async (req, res) => {
  try {
    ...
    if (tokenStore.has(token)) {
      ...
      fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Added a Comment on PostId(${postId}) , ${timeStr} \n`)
  ...
  if (tokenStore.has(token)) {
    ...
    if (user) {
      ...
      if (Object.keys(updateFields).length > 0) {
        ...
        fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Account Updated , ${timeStr} \n`)
        res.render("updatealumni", {
...
app.post("/instadmin", async (req, res) => {
  try {
    ...
    if (!iadm) {
      ...
      fs.appendFileSync('./uploads/Admin/Admins.csv', `${admid} , Institution Admin Logged in , ${timeStr} \n`)
  ...
  try {
    ...
    if (tokenStore.has(token)) {
      ...
      if (updated.modifiedCount > 0) {
        ...
        fs.appendFileSync('./uploads/Admin/Admins.csv', `${AdmToken} , Approved Rollno ${rollno} , ${timeStr} \n`)
  ...
  try {
    ...
    if (tokenStore.has(token)) {
      ...
      if (result.deletedCount === 1) {
        ...
        fs.appendFileSync('./uploads/Admin/Admins.csv', `${AdmToken} , Rejected Rollno ${rollno} , ${timeStr} ,Reamrk : ${remark}\n`)
...
app.post("/adminregistercomplaint", async (req, res) => {
  ...
  if (tokenStore.has(token)) {
    ...
    if (complaintscount < 2) {
      ...
      const complaintId = registered._id;
      fs.appendFileSync('./uploads/Admin/Admins.csv', `${iadmid} , Registered a Complaint CID:(${complaintId}) , ${timeStr} \n`)
...
app.post("/raisequery", async (req, res) => {
  ...
  if (tokenStore.has(token)) {
    ...
    console.log(registered);
    fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Raised a Query , ${timeStr} , Query : ${query}\n`)
    res.render("raisequery", {
...
app.post("/addqueryresponse", async (req, res) => {
  ...
  if (tokenStore.has(token)) {
    ...
    const respquery = await Query.findByIdAndUpdate(queryId, {
      $push: {
        replies: {
          name: user.name,
          rollno: rollno,
    ...
    if (respquery) {
      fs.appendFileSync('./uploads/Admin/Users.csv', `${rollno} , Replied to Query(${queryId}) , ${timeStr} , Response : ${comment} \n`)
...
app.post("/headadmin", async (req, res) => {
  try {
    ...
    if (!hadm) {
      ...
      fs.appendFileSync('./uploads/Admin/Admins.csv', `${admid} , Headadmin Logged in , ${timeStr} \n`)
...
app.post("/headadmin/addinstadm", async (req, res) => {
  ...
  if (tokenStore.has(token)) {
    ...
    if (registered) {
      ...
      fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Added Institution Admin ${employeeid} , ${timeStr} , Remark : ${remark}\n`)
  ...
  if (tokenStore.has(token)) {
    ...
    try {
      ...
      if (!updatedComplaint) {
        ...
        fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Replied to Institution Admin Complaint(${complaintId}) , ${timeStr} , Response : ${response} Status:${complaintStatus}\n`)
      }
...
app.post('/headadmin/editalumni/:rollno', async (req, res) => {
  ...
  const changed = await Register.updateOne({ rollno: roll }, { $set: updateFields });
  if (changed) {
    fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Edited Alumni Data (${rollno}) , ${timeStr} \n`)
...
app.get('/headadmin/mailalumni/:rollno', async (req, res) => {
  const token = req.cookies.headAdmToken;
  ...
  if (tokenStore.has(token)) {
    ...
    if (student) {
      res.render('headadminmailalumni', {
        name: student.name,
        rollno: student.rollno,
        mobno: student.mobno.mobno,
        branch: student.branch,
...
app.post('/headadmin/mailalumni/:rollno', async (req, res) => {
  ...
  if (tokenStore.has(token)) {
    ...
    const user = await Register.findOne({ rollno: rollno }, { mailid: 1 })
    const mailid = user.mailid
    ...
    const mailOptions = {
      ...
      if (error) {
        ...
        console.log("Email sent:", info.response);
        fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Mailed to Alumni(${rollno}) , ${timeStr} ,Message : ${msg}\n`)
...
app.get('/headadmin/massmailalumnis', async (req, res) => {
  const token = req.cookies.headAdmToken;

  if (tokenStore.has(token)) {
    res.render('headadminmassmail')

  } else {
    res.redirect('/headadmin');
  }
});
app.post('/headadmin/massmailalumnis', async (req, res) => {
  const token = req.cookies.headAdmToken;
  const hadmid = tokenStore.get(token);
  ...
  try {
    const alumniList = await Register.find({}, 'mailid'); // Fetch all alumni emails

    // Extract email addresses
    const emailList = alumniList.map(alumni => alumni.mailid);

    if (emailList.length === 0) {
      return res.render('headadminmailalumni', { warn: "No alumni found to send emails." });
    }
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.log("Error:", error);
        return res.render('headadminmailalumni', { warn: "Something went wrong." });
      } else {
        fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Sent a Mass Mail , ${timeStr} , Mail Text :${msg} \n`)
        console.log("Emails sent:", info.response);
        return res.render('headadminmailalumni', { success: "Mass email sent successfully!" });
      }
    });

  } catch (err) {
    console.error("Database error:", err);
    res.render('headadminmailalumni', { warn: "Error fetching alumni emails." });
  }
});
      ...
      fs.createReadStream(filePath)
        ...
        .on("end", async () => {
          try {
            ...
            fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Uploaded Alumni Verify Details , ${timeStr}  \n`)
...
app.post('/addalumni', async (req, res) => {
  try {
    ...
    if (registered) {
      fs.appendFileSync('./uploads/Admin/Admins.csv', `${hadmid} , Added an Alumni Verify Details(${roll}) , ${timeStr}  \n`)
...
app.get("/freeupspace", async (req, res) => {
  ...
  const directory3 = path.join(__dirname, "uploads/posts/");
  const token = req.cookies.headAdmToken;
  const hadmid = tokenStore.get(token);
